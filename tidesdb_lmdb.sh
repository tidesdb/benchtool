...
RESULTS="tidesdb_lmdb_benchmark_results_${TIMESTAMP}.txt"
CSV_FILE="tidesdb_lmdb_benchmark_results_${TIMESTAMP}.csv"
...
log "*------------------------------------------*"
log "RUNNER: TidesDB vs LMDB Comparison"
log "Date: $(date)"
...
run_comparison() {
    ...
    cleanup_db || exit 1
    log "Running TidesDB..."
    $BENCH -e tidesdb $bench_args $SYNC_FLAG $BTREE_FLAG -d "$DB_PATH" --test-name "$test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"

    cleanup_db || exit 1
    log "Running LMDB..."
    $BENCH -e lmdb $bench_args $SYNC_FLAG -d "$DB_PATH" --test-name "$test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
...
run_read_comparison() {
    ...
    cleanup_db || exit 1
    log "Populating LMDB for read test..."
    $BENCH -e lmdb $populate_args $SYNC_FLAG -d "$DB_PATH" --test-name "$populate_test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"

    log "Running LMDB read test..."
    $BENCH -e lmdb $read_args $SYNC_FLAG -d "$DB_PATH" --test-name "$test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
...
run_delete_comparison() {
    ...
    cleanup_db || exit 1
    log "Populating LMDB for delete test..."
    $BENCH -e lmdb $write_args $SYNC_FLAG -d "$DB_PATH" --test-name "$populate_test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"

    log "Running LMDB delete test..."
    $BENCH -e lmdb $delete_args $SYNC_FLAG -d "$DB_PATH" --test-name "$test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
...
run_seek_comparison() {
    ...
    cleanup_db || exit 1
    log "Populating LMDB for seek test..."
    $BENCH -e lmdb $populate_args $SYNC_FLAG -d "$DB_PATH" --test-name "$populate_test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"

    log "Running LMDB seek test..."
    $BENCH -e lmdb $seek_args $SYNC_FLAG -d "$DB_PATH" --test-name "$test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
...
run_range_comparison() {
    ...
    cleanup_db || exit 1
    log "Populating LMDB for range test..."
    $BENCH -e lmdb $populate_args $SYNC_FLAG -d "$DB_PATH" --test-name "$populate_test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"

    log "Running LMDB range test..."
    $BENCH -e lmdb $range_args $SYNC_FLAG -d "$DB_PATH" --test-name "$test_id" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
