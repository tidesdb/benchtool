...
######################################################################################
#
# TidesDB vs RocksDB Comprehensive Benchmark No Bloom & Block Indexes Suite
#
######################################################################################
...
RESULTS="tidesdb_rocksdb_no_bloom_indexes.txt"
CSV_FILE="tidesdb_rocksdb_no_bloom_indexes.csv"

SYNC_ENABLED="false"

DEFAULT_BATCH_SIZE=3

# Disable bloom filters and block indexes for this benchmark
...
echo "===================================" | tee -a "$RESULTS"
echo "TidesDB vs RocksDB Comparison (No Bloom/Block Indexes)" | tee -a "$RESULTS"
...
echo "Bloom Filters: DISABLED" | tee -a "$RESULTS"
echo "Block Indexes: DISABLED" | tee -a "$RESULTS"
...
run_comparison() {
    ...
    $BENCH -e tidesdb -c $bench_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" 2>&1 | tee -a "$RESULTS"
...
run_read_comparison() {
    ...
    $BENCH -e tidesdb $populate_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" 2>&1 | tee -a "$RESULTS"

    echo "Running TidesDB read test..." | tee -a "$RESULTS"
    $BENCH -e tidesdb $read_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"

    cleanup_db || exit 1
    echo "Populating RocksDB for read test..." | tee -a "$RESULTS"
    $BENCH -e rocksdb $populate_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" 2>&1 | tee -a "$RESULTS"

    echo "Running RocksDB read test..." | tee -a "$RESULTS"
    $BENCH -e rocksdb $read_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
...
run_delete_comparison() {
    ...
    $BENCH -e tidesdb $write_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" 2>&1 | tee -a "$RESULTS"

    echo "Running TidesDB delete test..." | tee -a "$RESULTS"
    $BENCH -e tidesdb $delete_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"

    cleanup_db || exit 1
    echo "Populating RocksDB for delete test..." | tee -a "$RESULTS"
    $BENCH -e rocksdb $write_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" 2>&1 | tee -a "$RESULTS"

    echo "Running RocksDB delete test..." | tee -a "$RESULTS"
    $BENCH -e rocksdb $delete_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
...
run_seek_comparison() {
    ...
    $BENCH -e tidesdb $seek_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
    ...
    $BENCH -e rocksdb $seek_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
...
run_range_comparison() {
    ...
    $BENCH -e tidesdb $range_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
    ...
    $BENCH -e rocksdb $range_args $SYNC_FLAG $NO_BLOOM_BLOCK_FLAGS -d "$DB_PATH" --csv "$CSV_FILE" 2>&1 | tee -a "$RESULTS"
...
run_comparison "Batch Size 1 (no batching, 45K ops)" \
    -w write -p random -o 45000 -t 6 -b 1

run_comparison "Batch Size 10 (45K ops)" \
    -w write -p random -o 45000 -t 6 -b 10

run_comparison "Batch Size 100 (45K ops)" \
    -w write -p random -o 45000 -t 6 -b 100

run_comparison "Batch Size 1000 (45K ops)" \
    -w write -p random -o 45000 -t 6 -b 1000

run_comparison "Batch Size 10000 (45K ops)" \
    -w write -p random -o 45000 -t 6 -b 10000

echo "### 10. Batch Size Impact on Deletes ###" | tee -a "$RESULTS"
run_delete_comparison "Delete Batch=1 (45K ops)" \
    -w delete -p random -o 45000 -t 6 -b 1

run_delete_comparison "Delete Batch=100 (45K ops)" \
    -w delete -p random -o 45000 -t 6 -b 100

run_delete_comparison "Delete Batch=1000 (45K ops)" \
    -w delete -p random -o 45000 -t 6 -b 1000

echo "### 11. Seek Performance ###" | tee -a "$RESULTS"
run_seek_comparison "Random Seek (45K ops, 1 thread)" \
    -w seek -p random -o 45000 -t 6

run_seek_comparison "Sequential Seek (45K ops, 1 thread)" \
    -w seek -p seq -o 45000 -t 6

run_seek_comparison "Zipfian Seek (45K ops, 1 thread)" \
    -w seek -p zipfian -o 45000 -t 6

echo "### 12. Range Query Performance ###" | tee -a "$RESULTS"
run_range_comparison "Range Scan 100 keys (45K ops, 1 thread)" \
    -w range -p random -o 45000 -t 6 --range-size 100

run_range_comparison "Range Scan 1000 keys (45K ops, 1 thread)" \
    -w range -p random -o 45000 -t 6 --range-size 1000

run_range_comparison "Sequential Range Scan 100 keys (45K ops, 1 thread)" \
